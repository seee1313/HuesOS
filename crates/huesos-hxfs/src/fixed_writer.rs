...
use crate::{
    parse_dir_record, parse_extent_record, parse_extent_record_v2, parse_extent_record_v2_legacy,
    parse_extent_tree_root, parse_header, parse_object_record, read_checkpoint, read_superblock,
    read_system_volume, validate_metadata_block, ExtentCompressionMeta, HxfsError,
    DIR_RECORD_BYTES, EXTENT_RECORD_BYTES, EXTENT_RECORD_BYTES_V2, HEADER_BYTES,
    LEGACY_EXTENT_RECORD_BYTES_V2, OBJECT_RECORD_BYTES,
};
use alloc::vec;
use alloc::vec::Vec;
...
mod transaction;
use transaction::{JournalCursor, TransactionShape};
...
> {
    ...
    /// Versioned descriptors persisted in the v6 encryption-policy root.
    encryption_policies: Vec<crate::crypto::EncryptionPolicy>,
    ...
    /// v5 compatibility mounts are read-only until an explicit migration.
    ...
    > FixedHxfsWriter<S, MAX_OBJECTS, MAX_DIR_ENTRIES, MAX_EXTENTS>
{
    ...
    /// interchangeable.
    /// Mount mutable HxFS using the versioned policy tables referenced by the
    /// checkpoint. This is the production service entry point.
    pub fn mount_from_disk(mut store: S, volume_key: Option<&[u8; 32]>) -> FixedResult<Self> {
    ...
    pub fn mount_with_keys(
        ...
        let mut mounted = Self {
            ...
            system_volume,
            encryption,
            encryption_policies: encryption_policies.to_vec(),
            #[cfg(feature = "crypto-aes-gcm")]
            metadata_key,
            #[cfg(feature = "crypto-aes-gcm")]
            extent_key,
            ...
            legacy_read_only: superblock.format_version == LEGACY_FORMAT_VERSION,
            dirty: false,
        };
    ...
    pub fn migrate_legacy_to_v6(
        ...
        let resolved = crate::resolve_mount_encryption(&self.system_volume, encryption_policies)?;
        if resolved != self.encryption {
            return Err(HxfsError::EncryptedPolicyInvalid);
        }
        if self.system_volume.compression_policy_id != 0
            && crate::compression::resolve_compression_policy(
                self.system_volume.compression_policy_id,
                compression_policies,
            )
            .is_err()
        {
        ...
        self.encryption_policies = encryption_policies.to_vec();
        self.compression_policies = compression_policies.to_vec();
        self.legacy_read_only = false;
        self.superblock.format_version = FORMAT_VERSION;
        self.superblock.type_system_version = TYPE_SYSTEM_VERSION;
        self.superblock.incompatible_features |= FEATURE_INCOMPAT_V6_POLICY_TABLES_AND_GENERATION;
        self.dirty = true;
        self.publish_checkpoint()
    }

    fn ensure_mutable_v6(&self) -> FixedResult<()> {
        if self.legacy_read_only {
    ...
    pub fn scrub_all(&mut self) -> FixedResult<(u64, u64)> {
        ...
        let mut check_single = |this: &mut Self, lba: u64, block_type: u32, owner: u64| {
            if lba == 0 {
                return;
            }
            let mut block = [0u8; BLOCK_SIZE];
            match this.read_mounted_metadata_block(lba, block_type, owner, &mut block) {
                Ok(_) => blocks += 1,
                Err(_) => errors += 1,
            }
        };
    ...
    pub fn publish_checkpoint(&mut self) -> FixedResult<u64> {
        ...
        let encryption_policy_tree_lba = quota_tree_lba.checked_add(1).ok_or(HxfsError::NoSpace)?;
        let compression_policy_tree_lba = encryption_policy_tree_lba
            .checked_add(1)
            .ok_or(HxfsError::NoSpace)?;
        // Stage F: an Hxblob build places its index + Merkle blocks between
        // the policy roots and the checkpoint. A build without Hxblob does not
        ...
        let (hxblob_index_tree_lba, hxblob_merkle_tree_lba, checkpoint_lba) = {
            let index = compression_policy_tree_lba
                .checked_add(1)
                .ok_or(HxfsError::NoSpace)?;
        ...
        let encryption_policy_block =
            self.build_encryption_policy_tree_block(encryption_policy_tree_lba)?;
        self.write_journaled_target(
            encryption_policy_tree_lba,
            &encryption_policy_block,
            sequence,
            journal.regular()?,
            record_count,
            journal_start_lba,
        ...
        let compression_policy_block =
            self.build_compression_policy_tree_block(compression_policy_tree_lba)?;
        self.write_journaled_target(
            compression_policy_tree_lba,
            &compression_policy_block,
            sequence,
            journal.regular()?,
            record_count,
            journal_start_lba,
    ...
    fn load_extents(&mut self, object: ObjectDescriptor) -> FixedResult<()> {
        ...
        while index < count {
            let (block_ref, offset) = if let Some((header, _block_type)) = single {
                ...
                (
                    leaf_buf.as_slice(),
                    HEADER_BYTES + within * versioned_record_bytes,
                )
            };
            let (extent, compression) = if record_bytes == versioned_record_bytes {
                if legacy {
    ...
    fn encryption_args(
        ...
        (
            ...
            self.metadata_key.as_ref().map(|key| &**key),
            &self.volume_uuid,
        )
    }
            ...
            if let Some(extent) = self.extents[index] {
                if extent.object_id == object_id {
                    ...
                    if v2 {
                        ...
                        // v6 keeps 36..40 reserved and persists the complete
                        // generation. No truncation is permitted at the AEAD
                        // nonce boundary.
                        payload[offset + 40..offset + 48]
                            .copy_from_slice(&extent.extent.generation.to_le_bytes());
                    }
                    written += 1;
                }
            }
    ...
    fn serialize_extent_record(
        ...
        if v2 {
            ...
            // See `build_extent_table_block`: v6 carries the complete
            // generation after the reserved word.
            payload[offset + 40..offset + 48]
                .copy_from_slice(&extent.extent.generation.to_le_bytes());
        }
    }
    ...
    fn build_encryption_policy_tree_block(&self, lba: u64) -> FixedResult<[u8; BLOCK_SIZE]> {
        let mut payload = [0u8; BLOCK_SIZE - HEADER_BYTES];
        let length = crate::policy_table::encode_encryption_payload(
    ...
    fn build_compression_policy_tree_block(&self, lba: u64) -> FixedResult<[u8; BLOCK_SIZE]> {
        let mut payload = [0u8; BLOCK_SIZE - HEADER_BYTES];
        let length = crate::policy_table::encode_compression_payload(
            &self.compression_policies,
            &mut payload,
        )?;
        Ok(make_metadata_block(
