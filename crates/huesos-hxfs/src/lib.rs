...
fn stage_e2_job_quota_round_trip() {
    ...
    let Ok(mut writer) = FixedHxfsWriter::<_, 16, 32, 64>::mount_with_policies(store, &[], &comps, None)
    else {
        assert!(false, "writer mount must succeed");
        return;
    };
    ...
    let Ok(mut fs) = FixedHxfsWriter::<_, 16, 32, 64>::mount_with_policies(remount, &[], &comps, None)
    else {
        assert!(false, "remount must succeed");
        return;
    };
...
fn stage_e2_scrub_all_reports_clean() {
    ...
    let Ok(mut writer) = FixedHxfsWriter::<_, 16, 32, 512>::mount_with_policies(store, &[], &comps, None)
    else {
        assert!(false, "writer mount must succeed");
        return;
    };
