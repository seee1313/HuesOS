...
### 8. Legacy ChannelRead Silent Truncation — ✅ FIXED

**Problem:** sys_channel_read truncates messages if user buffer is smaller than message size. Data is silently lost.

**Resolution:** Return BufferTooSmall error instead of truncating.

**Impact:** Correctness fix. Userspace can now detect undersized buffers.

---

### 9. Process Exit Waiter Count Mismatch — ✅ FIXED
...
**Resolution:** ExitWaiterGuard RAII automatically removes waiter on drop.

**Impact:** Correctness fix. Prevents process leaks on error paths.
...
| 🟡 P1 | #8 ChannelRead truncation | Medium | Low | ✅ FIXED |
| 🟢 P2 | #9 Exit waiter leak | Low | Low | ✅ FIXED |
...
- ✅ Service launch integration (acpi-manager)
- ✅ ChannelRead truncation fix
- ✅ ExitWaiterGuard RAII
