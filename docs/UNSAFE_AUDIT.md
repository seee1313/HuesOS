...
At the policy-hardening baseline, the repository contains 140 first-party Rust
files and 30,405 Rust lines. The measured surface is 220 unsafe blocks, 55
unsafe functions, 28 unsafe impls, one `static mut`, 25 unwrap calls, 22 expect
calls, and 5 panic macros. The previous `safety-budget.json` described an
older 129-file checkout and caused `make audit-check` to fail before this
review. The baseline was regenerated only after confirming that this change
adds no new unsafe site or panicking call; the remaining unsafe surface is
existing architecture, allocator, FFI, and executor code covered below.
