...
At the current policy-integration baseline, the repository contains 142
first-party Rust files and 31,619 Rust lines. The measured surface is 231
unsafe blocks, 57 unsafe functions, 28 unsafe impls, one `static mut`, 25
...
reviewing the additional paging, I/O APIC, and recoverable-copy blocks below;
no new unsafe site or panicking call was added elsewhere.
...
## Recoverable user-copy assembly boundary

The recoverable-copy integration adds the typed FFI wrappers and the kernel
linker-section reader for four assembly copy ranges. The wrappers are unsafe
because they require a validated destination/source, the Process copy lock, and
an active SMAP access guard. The assembly itself performs only byte load/store
operations, and every dereference has a sorted `.ex_table` entry whose fixup
returns `-1`; no general kernel fault is converted into recovery. The page-fault
handler consults the published lookup callback only for ring-0 faults and
otherwise enters the existing fatal path.

The linker symbols are checked for alignment and entry-size divisibility before
creating the borrowed `FixupRange` slice. No allocation or lock is taken in the
fault handler.
