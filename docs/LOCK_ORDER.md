...
`huesos-object`'s locks that are reachable from IRQ context (`REGISTRY`, `Port::packets`/`quota`, `Interrupt::binding`, `WaitQueue::waiters`/`async_wakers`, and the wait-timeout table) are additionally wrapped in a crate-local `IrqGuard` (`crates/huesos-object/src/irq_guard.rs`) that disables local interrupts for the critical section — the same pattern `huesos-pmm` already uses for its allocator lock. This closes the concrete single-CPU self-deadlock "the keyboard IRQ1 handler tries to lock the registry/Port/wait-queue while ordinary syscall-context code on the same CPU already holds it", which manifested as the system (or just the process reading the keyboard Port) freezing solid after a few seconds of real typing, with the local CPU's timer tick stopping and no panic message. See `docs/UNSAFE_AUDIT.md` § "huesos-object IRQ-guard boundary" for the root-cause writeup and the alternative (full `RankedIrqSafeTicketLock` migration) that remains the eventual target once `huesos-object` no longer needs to run under host tests without `huesos-arch`.
