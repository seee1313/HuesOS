...
The context-switch and ring0→ring3 assembly boundaries call `assert_no_ranked_locks_held`, so a ranked guard cannot silently survive a task switch or userspace entry. The allocator, paging state, architecture IRQ state, uACPI initialization, process startup records, per-CPU scheduler, task reaper, and process-teardown queues have been migrated. CI rejects any new `spin::Mutex` in the kernel, architecture, or in-kernel uACPI crates. Legacy `spin::Mutex` callback statics remain in the platform-neutral syscall crate (the object crate is fully migrated to `IrqSafeMutex` — see below); those locks are still governed by this table but do not yet provide runtime evidence. They must move to a host-testable shared ranked-lock core before runtime enforcement is considered complete.

Every shared-state field/static in `huesos-object` (the global registry, `Port`, `Interrupt`, `WaitQueue`, `Channel`, `Job`, `Process`, `Thread`, `Vmar`, `Vmo`, and every function-pointer callback static) uses `crate::irq_guard::IrqSafeMutex` (`crates/huesos-object/src/irq_guard.rs`), a `spin::Mutex`-compatible type that makes it impossible to `.lock()` without disabling local interrupts for the critical section — the same idea as `huesos-pmm`'s crate-local IRQ-mask guard, but enforced by the type system rather than by convention at each call site. `tools/check-huesos-object-lock-policy.py` (wired into `make audit-check`) rejects any new bare `spin::Mutex` in the crate to keep it that way. This closes the concrete single-CPU self-deadlock "the keyboard IRQ1 handler tries to lock the registry/Port/wait-queue/callback-pointer state while ordinary syscall-context code on the same CPU already holds it", which manifested as the system (or just the process reading the keyboard Port) freezing solid after a few seconds of real typing, with the local CPU's timer tick stopping and no panic message. An earlier, ad-hoc version of this fix wrapped only a manually curated subset of these locks and field-verified as still occasionally freezing (having missed the callback-pointer statics in `wait.rs`), which is why the type-enforced version replaced it. See `docs/UNSAFE_AUDIT.md` § "huesos-object migration to type-enforced `IrqSafeMutex`" for the root-cause writeup and the alternative (full `RankedIrqSafeTicketLock` migration) that remains the eventual target once `huesos-object` no longer needs to run under host tests without `huesos-arch`.
